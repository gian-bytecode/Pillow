...
from PIL.gpu import ImageFilter as GpuImageFilter
...
def make_images(size, mode="RGB"):
    ...
    cpu = Image.new(mode, (size, size), (128, 64, 192) if mode == "RGB" else
                    (128, 64, 192, 255) if mode == "RGBA" else 128)
    # Add some variation
    pixels = cpu.load()
...
def benchmark_transfer(sizes):
    ...
    for s in sizes:
        ...
        print(f"  Transfer {s}x{s}: up={t_up:.3f}ms  down={t_down:.3f}ms  rt={t_rt:.3f}ms")
    return results
...
def run_benchmarks():
    ...
    benchmarks.append(("blend_0.5",
        "BLEND",
        "BLEND",
        "RGB"))
    
    benchmarks.append(("alpha_composite",
        "ALPHA_COMP",
        "ALPHA_COMP",
        "RGBA"))

    # --- Histogram ---
    benchmarks.append(("histogram",
        lambda c: c.histogram(),
        lambda g: g.histogram(),
        "L"))
    
    benchmarks.append(("histogram_RGB",
        lambda c: c.histogram(),
        lambda g: g.histogram(),
        "RGB"))

    # --- Getbbox / Getextrema ---
    benchmarks.append(("getbbox",
        lambda c: c.getbbox(),
        lambda g: g.getbbox(),
        "L"))
    
    benchmarks.append(("getextrema",
        lambda c: c.getextrema(),
        lambda g: g.getextrema(),
        "L"))

    # --- Crop / Expand / Offset ---
    benchmarks.append(("crop_quarter",
        lambda c: c.crop((c.width // 4, c.height // 4, 3 * c.width // 4, 3 * c.height // 4)),
        lambda g: g.crop((g.width // 4, g.height // 4, 3 * g.width // 4, 3 * g.height // 4)),
        "RGB"))
    
    benchmarks.append(("expand_10px",
        lambda c: ImageOps.expand(c, 10, fill=(0, 0, 0)),
        lambda g: g.expand(10, fill=0),
        "RGB"))

    benchmarks.append(("offset",
        lambda c: ImageChops.offset(c, 50, 50),
        lambda g: g.offset(50, 50),
        "RGB"))

    # --- Split / GetChannel ---
    benchmarks.append(("split",
        lambda c: c.split(),
        lambda g: g.split(),
        "RGB"))
    
    benchmarks.append(("getchannel_0",
        lambda c: c.getchannel(0),
        lambda g: g.getchannel(0),
        "RGB"))

    # --- Effect spread ---
    benchmarks.append(("effect_spread_5",
        lambda c: c.effect_spread(5),
        lambda g: g.effect_spread(5),
        "RGB"))

    # --- Reduce ---
    benchmarks.append(("reduce_2x",
        lambda c: c.reduce(2),
        lambda g: g.reduce(2),
        "RGB"))

    benchmarks.append(("reduce_4x",
        lambda c: c.reduce(4),
        lambda g: g.reduce(4),
        "RGB"))

    # --- Rank filter ---
    benchmarks.append(("rank_filter_median_3",
        lambda c: c.filter(ImageFilter.MedianFilter(3)),
        lambda g: g.rank_filter(3, 4),
        "L"))
    
    benchmarks.append(("rank_filter_median_5",
        lambda c: c.filter(ImageFilter.MedianFilter(5)),
        lambda g: g.rank_filter(5, 12),
        "L"))

    # --- Mode filter ---
    benchmarks.append(("mode_filter_3",
        lambda c: c.filter(ImageFilter.ModeFilter(3)),
        lambda g: g.mode_filter(3),
        "L"))

    # --- ImageOps ---
    benchmarks.append(("imageops_autocontrast",
        lambda c: ImageOps.autocontrast(c),
        lambda g: GpuImageOps.autocontrast(g),
        "RGB"))
    
    benchmarks.append(("imageops_equalize",
        lambda c: ImageOps.equalize(c),
        lambda g: GpuImageOps.equalize(g),
        "RGB"))

    # --- ImageEnhance ---
    benchmarks.append(("enhance_brightness_1.5",
        "BRIGHTNESS",
        "BRIGHTNESS",
        "RGB"))
    
    benchmarks.append(("enhance_contrast_1.5",
        "CONTRAST",
        "CONTRAST",
        "RGB"))
    
    benchmarks.append(("enhance_color_0.5",
        "COLOR",
        "COLOR",
        "RGB"))
    
    benchmarks.append(("enhance_sharpness_2.0",
        "SHARPNESS",
        "SHARPNESS",
        "RGB"))

    # --- Gradients ---
    benchmarks.append(("linear_gradient",
        "LINGRAD",
        "LINGRAD",
        "L"))

    benchmarks.append(("radial_gradient",
        "RADGRAD",
        "RADGRAD",
        "L"))
    ...
    # --- Paste ---
    benchmarks.append(("paste",
        "PASTE",
        "PASTE",
        "RGBA"))

    # --- Transform (affine) ---
    benchmarks.append(("transform_affine",
        "AFFINE",
        "AFFINE",
        "RGB"))
    ...
    for bname, cpu_fn, gpu_fn, mode in benchmarks:
        ...
        for s in SIZES:
            ...
            if cpu_fn == "BLEND":
                ...
                t_cpu = timeit(lambda: cpu_img.transform(
                    cpu_img.size, Image.Transform.AFFINE,
                    coeffs, Image.Resampling.BILINEAR))
                t_gpu = timeit(lambda: gpu_img.transform(
                    gpu_img.size, 0, coeffs, 2, 1))
            else:
            ...
            print(f"  {s:5d}x{s:<5d}  CPU: {t_cpu:8.3f}ms  GPU: {t_gpu:8.3f}ms  speedup: {speedup:.2f}x")
...
def generate_report(results):
    ...
    lines.append(f"**Timing:** median of {REPEATS} runs after {WARMUP} warmup iterations\n")
    lines.append("")
    ...
    if "transfer" in results:
        for s in SIZES:
            ...
            lines.append(f"| {s}×{s} | {d['upload_ms']:.3f} | {d['download_ms']:.3f} | {d['roundtrip_ms']:.3f} |")
    lines.append("")

    # Group operations by category
    categories = {
        "Basic Operations": ["copy", "convert_RGB_to_L", "convert_L_to_RGB", "convert_RGB_to_RGBA"],
        "Resize": ["resize_half_nearest", "resize_half_bilinear", "resize_double_bilinear"],
        "Transpose / Transform": ["transpose_flip_lr", "transpose_rotate_90", "transform_affine"],
        "Blur": ["gaussian_blur_r2", "gaussian_blur_r5", "box_blur_r2", "box_blur_r5", "unsharp_mask"],
        "Convolution": ["convolve_sharpen"],
        "Point Operations": ["point_lut_invert", "point_transform", "negative", "posterize_4", "solarize_128"],
        "Compositing": ["blend_0.5", "alpha_composite", "paste"],
        "Statistics / Analysis": ["histogram", "histogram_RGB", "getbbox", "getextrema"],
        "Geometry": ["crop_quarter", "expand_10px", "offset"],
        "Channel Operations": ["split", "getchannel_0"],
        "Rank / Mode Filters": ["rank_filter_median_3", "rank_filter_median_5", "mode_filter_3"],
        "Reduction": ["reduce_2x", "reduce_4x"],
        "Effects": ["effect_spread_5"],
        "ImageOps": ["imageops_autocontrast", "imageops_equalize"],
        "ImageEnhance": ["enhance_brightness_1.5", "enhance_contrast_1.5", "enhance_color_0.5", "enhance_sharpness_2.0"],
    ...
    for cat_name, ops in categories.items():
        ...
        # Header
        size_headers = " | ".join(f"CPU | GPU | ×" for _ in SIZES)
        size_labels = " | ".join(f"{s}×{s} || " for s in SIZES)
        ...
        for op in valid_ops:
            ...
            for s in SIZES:
                ...
                row += f" {cpu_ms:.2f} | {gpu_ms:.2f} | {marker}{speedup:.1f}x{marker} |"
            lines.append(row)
    ...
    lines.append("The crossover point is the smallest image size where GPU compute (excluding transfer) is faster than CPU.\n")
    ...
    for cat_name, ops in categories.items():
        for op in ops:
            ...
            all_faster = all(results[op].get(s, {}).get("speedup", 0) > 1.0 for s in SIZES)
            if all_faster:
    ...
    lines.append("- **GPU (ms)**: Median execution time using GPU compute only (data already on GPU)")
    lines.append("- **×**: Speedup factor (CPU time / GPU time). Values > 1.0 mean GPU is faster")
    lines.append("- **Bold** speedup values indicate ≥ 2× improvement")
    lines.append("- Transfer overhead (Section 1) is NOT included in GPU times. For end-to-end")
    lines.append("  comparison, add the round-trip transfer time to GPU times")
    lines.append("")
