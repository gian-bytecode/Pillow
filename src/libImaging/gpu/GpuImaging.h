...
struct GPUBackendInstance {
    ...
    /* Blur / Filter */
    int (*gaussian_blur)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                         float xradius, float yradius, int passes);
    int (*box_blur)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                    float xradius, float yradius, int n);
    int (*unsharp_mask)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                        float radius, int percent, int threshold);
    int (*filter)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                  int ksize_x, int ksize_y, const float *kernel,
                  float divisor, float offset);

    /* Resample */
    int (*resample)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                    int xsize, int ysize, int filter, const float box[4]);

    /* Color conversion */
    int (*convert)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                   ModeID to_mode);

    /* Compositing */
    int (*blend)(GPUBackend self, ImagingGPU out,
                 ImagingGPU im1, ImagingGPU im2, float alpha);
    int (*alpha_composite)(GPUBackend self, ImagingGPU out,
                           ImagingGPU im1, ImagingGPU im2);

    /* Channel operations */
    int (*chop)(GPUBackend self, ImagingGPU out,
                ImagingGPU im1, ImagingGPU im2,
                int op, float scale, int offset);

    /* Geometry */
    int (*transpose)(GPUBackend self, ImagingGPU out, ImagingGPU in, int op);
    int (*transform)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                     int method, double a[8], int filter, int fill);

    /* Point / LUT */
    int (*point_lut)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                     const UINT8 *lut, int bands);
    int (*point_transform)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                           double scale, double offset);

    /* Color matrix */
    int (*color_matrix)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                        const float *matrix, int ncolumns);

    /* Color LUT 3D */
    int (*color_lut_3d)(GPUBackend self, ImagingGPU out, ImagingGPU in,
    ...
    /* Paste */
    int (*paste)(GPUBackend self, ImagingGPU dest, ImagingGPU src,
    ...
    /* Extended operations */
    int (*crop)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                int x0, int y0, int x1, int y1);
    int (*expand)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                  int xmargin, int ymargin, const UINT8 *fill);
    int (*offset)(GPUBackend self, ImagingGPU out, ImagingGPU in,
    ...
    int (*equalize)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                    const UINT8 *lut);
    ...
    int (*reduce)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                  int factor_x, int factor_y);
    int (*rank_filter)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                       int ksize, int rank);
    int (*mode_filter)(GPUBackend self, ImagingGPU out, ImagingGPU in,
                       int ksize);
